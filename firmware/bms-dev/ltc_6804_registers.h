...
struct ltc6804_cfg_reg_s {
    uint8_t             gpio;       ///< GPIO Pin Control (bit 0->pin1, bit 4->pin5)
    uint8_t             refon;      ///< Reference Powered Up
    uint8_t             swtrd;      ///< SWTEN Pin Status
    ...
    uint16_t            dcc;        ///< Discharge Cell x flag (1-12)
...
struct ltc6804_cell_voltage_reg_s {
    uint16_t            offset[3];  ///< Group offset 0-3
    enum ltc6804_PEC_e  pecOk;      ///< PEC status for received register data
};
...
struct ltc6804_cell_voltage_s {
    uint16_t            cell[CELL_COUNT_PER_IC];    ///< Cells 0-11 (1-12 on '6804)
    enum ltc6804_PEC_e  pecOk;                      ///< PEC status for received register data
};
...
struct ltc6804_aux_reg_a_s {
    uint16_t            g1v;        ///< GPIO 1 Voltage
    uint16_t            g2v;        ///< GPIO 2 Voltage
    uint16_t            g3v;        ///< GPIO 3 Voltage
    enum ltc6804_PEC_e  pecOk;      ///< PEC status for received register data
};
...
struct ltc6804_aux_reg_b_s {
    uint16_t            g4v;        ///< GPIO 4 Voltage
    uint16_t            g5v;        ///< GPIO 5 Voltage
    uint16_t            ref;        ///< Second reference voltage
    enum ltc6804_PEC_e  pecOk;      ///< PEC status for received register data
};
...
struct ltc6804_stat_reg_a_s {
    ...
    uint16_t            itmp;       ///< Internal IC Die Temperature
...
struct ltc6804_stat_reg_b_s {
    ...
    uint8_t             muxfail;    ///< Multiplexer Self-Test Result
    uint8_t             thsd;       ///< Thermal Shutdown Status
    enum ltc6804_PEC_e  pecOk;      ///< PEC status for received register data
};
