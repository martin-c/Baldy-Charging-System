...
void ltc6804_util_encodeCommand(uint8_t addr, enum ltc6804_command_codes_e cmd, uint8_t *b)
{
    ...
    uint8_t cmdHi = (uint8_t)(cmd >> 8);      // CMD hi byte
    uint8_t cmdLo = (uint8_t)cmd;             // CMD lo byte
...
void ltc6804_util_decodeStatA(void *g, uint8_t *b)
{
    ...
    // copy bytes
    r->soc = b[0] | (uint16_t)b[1] << 8;
    r->itmp = b[2] | (uint16_t)b[3] << 8;
    r->va = b[4] | (uint16_t)b[5] << 8;
}
...
void ltc6804_util_decodeCfg(void *g, uint8_t *b)
{
    ...
    r->swtrd = (b[0] >> 1) & 0x01;
    r->adcopt = b[0] & 0x01;
    r->vuv = b[1] | ((uint16_t)(b[2] & 0x0F)) << 8;
    r->vov = (b[2] & 0xF0) >> 4 | ((uint16_t)b[3]) << 4;
...
void ltc6804_util_encodeCfg(uint8_t *b, void *g) {
    ...
    b[1] = (uint8_t)r->vuv;                    // VUV lo
    b[2] = (uint8_t)(r->vuv >> 8) |            // VUV hi
            ((uint8_t)r->vov) << 4;            // VOV lo nibble
    b[3] = (uint8_t)(r->vov >> 4);             // VOV hi
    b[4] = (uint8_t)r->dcc;
